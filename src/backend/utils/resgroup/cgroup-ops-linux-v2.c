...
static void create_default_cpuset_group_beta(void);
static int64 get_cfs_period_us_beta();
...
init_cpuset_beta(void)
{
	...
	create_default_cpuset_group_beta();
}

static int64
get_cfs_period_us_beta()
{
...
checkcgroup_v2(void)
{
	 ...
	 */
	if (!cgroupSystemInfoBeta.cgroup_dir[0])
	...
	cgroupSystemInfoBeta.ncores = getCPUCores();

	cfs_period_us = get_cfs_period_us_beta();
	system_cfs_quota_us = cfs_period_us * cgroupSystemInfoBeta.ncores;
}
 ...
 */
static void
create_default_cpuset_group_beta(void)
{
	CGroupComponentType component = CGROUP_COMPONENT_PLAIN;
	int retry = 0;
...
static CGroupOpsRoutine cGroupOpsRoutineBeta = {
...
CGroupOpsRoutine *get_group_routine_beta(void)
{
	return &cGroupOpsRoutineBeta;
}

CGroupSystemInfo *get_cgroup_sysinfo_beta(void)
{
	return &cgroupSystemInfoBeta;
}
